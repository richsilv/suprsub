clientFunctions = (function() {
	...
	var initialize = function(circle) {
		...
		if (Router.current().route.name === "teamDetails") {
			...
			if (Meteor.user() && Meteor.user().profile && Meteor.user().profile.player && Meteor.user().profile.player.center) {
				...
				navigator.geolocation.getCurrentPosition(function(res) {
					...
					pitchMap.setCenter(defaultLocation);
					updateCircle();
					pitchesWithin({"lat": res.coords.latitude, "lng": res.coords.longitude}, 8000, function(err, res) {
		...
		if (appVars.circleSize) pitchesWithin({"lat": parseFloat(appVars.mapCenter.get().lat(), 10), "lng": parseFloat(appVars.mapCenter.get().lng(), 10)}, appVars.circleSize.get(), function(err, res) {
	...
	var pitchesWithin = function(center, distance, callback) {
		var ratio = 6.283184 / 360,
			...
			results = _.filter(Pitches.find({}, {sort: {prettyLocation: 1}}).fetch(), function(p) {
